...
void check (const Shape& shape, double area, int axis, bool reverse) {
    Rings rings; Point normal{}; normal[axis]=reverse?-1:1;
    int a=(axis+1)%3,b=(axis+2)%3;
    std::vector<Point> flat;
    for (auto ring:shape) {
        if (reverse) std::reverse(ring.begin(),ring.end());
        rings.emplace_back();
        for (auto xy:ring) { Point p{}; p[axis]=3; p[a]=xy[0]+100000; p[b]=xy[1]-100000; rings.back().push_back(p); flat.push_back(p); }
    }
    auto indices=Triangulate(rings,normal); double sum=0;
